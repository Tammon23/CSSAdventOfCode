...
def clean_input(t: str) -> tuple[str, int, int, int, int, int]:
    ingredient, _, capacity, _, durability, _, flavour, _, texture, _, calories = re.sub(r"[:,]", "", t).split(" ")
    return ingredient, int(capacity), int(durability), int(flavour), int(texture), int(calories)
